...
force_field *force_field_new() {
    force_field *p_force_field = calloc(1, sizeof(force_field));
...
float force_field_gravity(force_field *p_force_field, float mass1, float mass2, float distance) {
    return p_force_field->gravity_constant * mass1 * mass2 / (distance * distance);
}

float force_field_custom(force_field *p_force_field, float q1, float q2, float distance) {
    return -log(distance/1.5) * 0.01;
}

void force_field_random_custom(force_field *p_force_field, glmc_vec3 *random_force) {
    glmc_vec3_init_random(random_force, -1.0, 1.0);
    glmc_vec3_divide(random_force, 1000.0, random_force);
}

void force_field_centering(force_field *p_force_field, glmc_vec3 *centering_force, glmc_vec3 *position) {
