...
void renderobject_init(t_renderobject *p_renderobject, t_mesh *p_mesh, unsigned int a_offsets_size) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(t_vertex) * p_renderobject->mesh->a_vertex_size, p_renderobject->mesh->a_vertex, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, p_renderobject->ibo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * p_renderobject->mesh->a_indices_size, p_renderobject->mesh->a_indices, GL_STATIC_DRAW);
...
void renderobject_draw(t_renderobject *p_renderobject, t_program *p_program) {
    ...
    glBindVertexArray(p_renderobject->vao);
    glBindBuffer(GL_ARRAY_BUFFER, p_renderobject->vbo_instances);
    glDrawElementsInstanced(GL_TRIANGLES, p_renderobject->mesh->a_indices_size, GL_UNSIGNED_INT, 0, p_renderobject->a_offsets_size);
    glBindVertexArray(0);
    glUseProgram(0);
}
